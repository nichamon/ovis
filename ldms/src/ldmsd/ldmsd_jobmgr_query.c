			...
			char **error)
{
	...
	for (jm = ldmsd_jobmgr_first(); jm; jm = ldmsd_jobmgr_next(jm)) {
		ent = calloc(1, sizeof(*ent));
		if (!ent) {
			ldmsd_cfg_unlock(LDMSD_CFGOBJ_JOBMGR);
			goto err_2;
		}
		ldmsd_jobmgr_get(jm, "qmgr_ent");
		ent->mgr = jm;
		rc = ent->mgr->api->on_query_new((ldmsd_plug_handle_t)jm, q, &ent->qmgr_ctxt);
		if (rc) {
			ldmsd_cfg_unlock(LDMSD_CFGOBJ_JOBMGR);
			errno = rc;
			free(ent);
 ...
 err_2:
	while ((ent = TAILQ_FIRST(&q->mgr_tq))) {
		...
		ent->mgr->api->on_query_free((ldmsd_plug_handle_t)(ent->mgr), q, ent->qmgr_ctxt);
		free(ent);
	}
 err_1:
...
void qev_cb(ovis_event_t oev)
{
	...
	/* done with the event */
	mgr->api->qev_done(jev->qev.mgr, event_type, jev->ctxt);
