...
int auth_disable(ldmsd_cfgobj_t obj)
{
	ldmsd_auth_t auth;
...
json_entity_t auth_update(ldmsd_cfgobj_t obj, short enabled, json_entity_t dft,
								json_entity_t spc)
{
	int rc;
...
int auth_init(ldmsd_daemon_t obj)
{
	obj->attr = json_dict_build(NULL, JSON_STRING_VALUE, "name",
						LDMSD_AUTH_DOMAIN_NONE, -1);
	if (!obj->attr)
		return ENOMEM;
...
static struct daemon_obj_entry handler_tbl[] = {
	...
	{ "default_auth",	0,			auth_init,
	  auth_update,		NULL,			auth_disable
	},
...
json_entity_t ldmsd_daemon_query(ldmsd_cfgobj_t obj)
{
	...
	if (!query)
		goto oom;
	rc = json_dict_append(query, daemon->attr);
	if (rc)
		goto oom;
	return ldmsd_result_new(0, NULL, query);
oom:
