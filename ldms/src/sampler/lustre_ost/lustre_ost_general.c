...
int ost_general_schema_init(comp_id_t cid)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "ost_general_schema_init()\n");
	...
	if (rc) {
        ...
        return 0;
err2:
	ovis_log(mylog, OVIS_LERROR, SAMP ": lustre_ost_general schema creation failed to add %s. (%s)\n",
		field, STRERROR(-rc));
        ldms_schema_delete(sch);
err1:
        ovis_log(mylog, OVIS_LERROR, "lustre_ost_general schema creation failed\n");
        return -1;
}

void ost_general_schema_fini()
{
        ovis_log(mylog, OVIS_LDEBUG, "ost_general_schema_fini()\n");
        if (ost_general_schema != NULL) {
...
char *ost_general_osd_path_find(const char *search_path, const char *ost_name)
{
        ...
        if (osd_path != NULL) {
                ovis_log(mylog, OVIS_LDEBUG, "for ost %s found osd path %s\n",
                       ost_name, osd_path);
        } else {
                ovis_log(mylog, OVIS_LWARNING, "osd for ost %s not found\n",
                       ost_name);
        }
...
static uint64_t file_read_uint64_t(const char *dir, const char *file)
{
        ...
        if (fgets(valbuf, sizeof(valbuf), fp) == NULL) {
                ovis_log(mylog, OVIS_LWARNING, "unable to read %s\n", filepath);
                fclose(fp);
                return 0;
        }
        fclose(fp);
			      ...
			      const comp_id_t cid)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "ost_general_create()\n");
                                   ...
                                   ldms_set_t general_metric_set)
{
        ...
        if (sf == NULL) {
                ovis_log(mylog, OVIS_LWARNING, "file %s not found\n",
                       stats_path);
                return;
        }
        ...
        if (fgets(buf, sizeof(buf), sf) == NULL) {
                ovis_log(mylog, OVIS_LWARNING, "failed on read from %s\n",
                       stats_path);
                goto out1;
        }
        if (strncmp("snapshot_time", buf, sizeof("snapshot_time")-1) != 0) {
                ovis_log(mylog, OVIS_LWARNING, "first line in %s is not \"snapshot_time\": %s\n",
                       stats_path, buf);
                goto out1;
        }
        ...
        while (fgets(buf, sizeof(buf), sf)) {
                ...
                if (rc == 2) {
                        ...
                        if (index == -1) {
                                ovis_log(mylog, OVIS_LWARNING, "obdfilter stats metric not found: %s\n",
                                       str1);
                        } else {
...
{
        ovis_log(mylog, OVIS_LDEBUG, "ost_general_sample() %s\n",
               ost_name);
