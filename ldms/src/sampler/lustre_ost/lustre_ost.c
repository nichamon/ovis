...
static struct ost_data *ost_create(const char *ost_name, const char *basedir)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "ost_create() %s from %s\n",
               ost_name, basedir);
...
{
        ovis_log(mylog, OVIS_LDEBUG, "ost_destroy() %s\n", ost->name);
...
static void osts_refresh()
{
        ...
        if (dir == NULL) {
                ovis_log(mylog, OVIS_LDEBUG, "unable to open obdfilter dir %s\n",
                       OBDFILTER_PATH);
                return;
        }
...
static int sample(struct ldmsd_sampler *self)
{
        ...
        if (ost_job_stats_schema_is_initialized() < 0) {
                if (ost_job_stats_schema_init() < 0) {
                        ovis_log(mylog, OVIS_LERROR, "job stats schema create failed\n");
                        return ENOMEM;
                }
        }
