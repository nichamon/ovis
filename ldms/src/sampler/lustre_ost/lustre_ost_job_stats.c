...
err1:
        ovis_log(mylog, OVIS_LERROR, "lustre_ost_job_stats schema creation failed\n");
        return -1;
}

void ost_job_stats_schema_fini()
{
        ovis_log(mylog, OVIS_LDEBUG, "ost_job_stats_schema_fini()\n");
        if (ost_job_stats_schema != NULL) {
                                                            ...
                                                            const char *ost_name)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "ost_job_stats_data_create() jobid=%s\n",
               jobid);
...
out2:
        free(job_stats);
out1:
        ovis_log(mylog, OVIS_LERROR, "ost_job_stats_data_create failed\n");
        return NULL;
}

static void ost_job_stats_data_destroy(struct ost_job_stats_data *job_stats)
{
        ovis_log(mylog, OVIS_LDEBUG, "ost_job_stats_data_destroy() jobid=%s\n",
               job_stats->jobid);
                          ...
                          struct rbt *job_stats_tree)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "ost_job_stats_sample() %s\n",
               ost_name);
        ...
        if (strncmp("job_stats:", buf, sizeof("job_stats:")-1) != 0) {
                ovis_log(mylog, OVIS_LWARNING, "first line in %s is not \"job_stats:\": %s\n",
                       job_stats_path, buf);
                goto out1;
        }
        ...
        while (fgets(buf, sizeof(buf), js)) {
                ...
                if (rc == 1) {
                        ...
                        if (job_stats->prev_snapshot_time == val1)
                                continue;
                        ovis_log(mylog, OVIS_LDEBUG, "jobid %s has updated data\n",
                               job_stats->jobid);
                        ...
                        if (index == -1) {
                                ovis_log(mylog, OVIS_LWARNING, "ost job_stats metric not found: snapshot_time\n",
                                       str1);
                        } else {
                ...
                if (rc == 2) {
                        ...
                        if (index == -1) {
                                ovis_log(mylog, OVIS_LWARNING, "ost job_stats metric not found: %s\n",
                                       str1);
                        } else {
                                ldms_metric_set_u64(job_stats->metric_set, index, val1);
                        }
