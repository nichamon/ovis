...
static struct mdt_data *mdt_create(const char *mdt_name, const char *basedir)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, SAMP" mdt_create() %s from %s\n",
               mdt_name, basedir);
        ...
        if (strtok_r(mdt->fs_name, "-", &state) == NULL) {
                ovis_log(mylog, OVIS_LWARNING, SAMP" unable to parse filesystem name from \"%s\"\n",
                       mdt->fs_name);
                goto out7;
        }
...
{
        ovis_log(mylog, OVIS_LDEBUG, SAMP" mdt_destroy() %s\n", mdt->name);
...
static void mdts_refresh()
{
        ...
        if (dir == NULL) {
                ovis_log(mylog, OVIS_LDEBUG, SAMP" unable to open obdfilter dir %s\n",
                       MDT_PATH);
                return;
        }
...
static int sample(struct ldmsd_sampler *self)
{
        ...
        if (mdt_general_schema_is_initialized() < 0) {
                if (mdt_general_schema_init(&cid) < 0) {
                        ovis_log(mylog, OVIS_LERROR, SAMP" general schema create failed\n");
                        return ENOMEM;
                }
        }
        if (mdt_job_stats_schema_is_initialized() < 0) {
                if (mdt_job_stats_schema_init() < 0) {
                        ovis_log(mylog, OVIS_LERROR, SAMP" job stats schema create failed\n");
                        return ENOMEM;
                }
        }
...
struct ldmsd_plugin *get_plugin()
{
	...
	ovis_log(mylog, OVIS_LDEBUG, SAMP" get_plugin() called ("PACKAGE_STRING")\n");
