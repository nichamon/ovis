...
int mdt_general_schema_init(comp_id_t cid)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "mdt_general_schema_init()\n");
	...
	if (rc) {
        ...
        return 0;
err2:
	ovis_log(mylog, OVIS_LERROR, "lustre_mdt_general schema creation failed to add %s. (%s)\n",
		field, STRERROR(-rc));
        ldms_schema_delete(sch);
err1:
        ovis_log(mylog, OVIS_LERROR, "lustre_mdt_general schema creation failed\n");
        return -1;
}

void mdt_general_schema_fini()
{
        ovis_log(mylog, OVIS_LDEBUG, "mdt_general_schema_fini()\n");
        if (mdt_general_schema != NULL) {
...
char *mdt_general_osd_path_find(const char *search_path, const char *mdt_name)
{
        ...
        if (osd_path != NULL) {
                ovis_log(mylog, OVIS_LDEBUG, "for mdt %s found osd path %s\n",
                       mdt_name, osd_path);
        } else {
                ovis_log(mylog, OVIS_LWARNING, "osd for mdt %s not found\n",
                       mdt_name);
        }
			      ...
			      const comp_id_t cid)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, "mdt_general_create()\n");
                                   ...
                                   ldms_set_t general_metric_set)
{
        ...
        while (fgets(buf, sizeof(buf), sf)) {
                ...
                if (rc == 2) {
                        ...
                        if (index == -1) {
                                ovis_log(mylog, OVIS_LWARNING, "mdt md_stats metric not found: %s\n",
                                       str1);
                        } else {
...
{
        ovis_log(mylog, OVIS_LDEBUG, "mdt_general_sample() %s\n",
               mdt_name);
