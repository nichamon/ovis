...
int llite_general_schema_init(comp_id_t cid)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, SAMP ": llite_general_schema_init()\n");
        sch = ldms_schema_new("lustre_client");
        if (sch == NULL) {
		ovis_log(mylog, OVIS_LERROR, SAMP ": lustre_llite_general schema new failed"
			" (out of memory)\n");
                goto err1;
	}
	const char *field;
	...
	field = "llite";
        ...
        return 0;
err2:
	ovis_log(mylog, OVIS_LERROR, SAMP ": lustre_llite_general schema creation failed to add %s. (%s)\n",
		field, STRERROR(-rc));
...
{
        ovis_log(mylog, OVIS_LDEBUG, SAMP ": llite_general_schema_fini()\n");
        if (llite_general_schema != NULL) {
				...
				const struct base_auth *auth)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, SAMP ": llite_general_create()\n");
                                   ...
                                   ldms_set_t general_metric_set)
{
	...
	int ec = 0;
        ...
        if (strncmp("snapshot_time", buf, sizeof("snapshot_time")-1) != 0) {
                ovis_log(mylog, OVIS_LWARNING, SAMP ": first line in %s is not \"snapshot_time\": %s\n",
                       stats_path, buf);
		ec = ENOMSG;
                goto out1;
        }
	...
	jobid_helper_metric_update(general_metric_set);
        while (fgets(buf, sizeof(buf), sf)) {
                ...
                if (rc == 2) {
                        ...
                        if (index == -1) {
                                ovis_log(mylog, OVIS_LWARNING, SAMP ": llite stats metric not found: %s\n",
                                       str1);
                        } else {
...
void llite_general_sample(const char *llite_name, const char *stats_path,
                          ldms_set_t general_metric_set)
{
        ovis_log(mylog, OVIS_LDEBUG, SAMP ": llite_general_sample() %s\n",
               llite_name);
