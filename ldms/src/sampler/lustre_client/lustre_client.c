...
static struct llite_data *llite_create(const char *llite_name, const char *basedir)
{
        ...
        ovis_log(mylog, OVIS_LDEBUG, " llite_create() %s from %s\n",
               llite_name, basedir);
        ...
        if (strtok_r(llite->fs_name, "-", &state) == NULL) {
                ovis_log(mylog, OVIS_LWARNING, "unable to parse filesystem name from \"%s\"\n",
                       llite->fs_name);
                goto out6;
        }
...
{
        ovis_log(mylog, OVIS_LDEBUG, "llite_destroy() %s\n", llite->name);
...
static const char *const find_llite_path()
{
        ...
        for (i = 0; i < llite_paths_len; i++) {
                ...
                if (previously_found_path != llite_paths[i]) {
                        ...
                        ovis_log(mylog, OVIS_LDEBUG, "find_llite_path() found %s\n",
                               llite_paths[i]);
                }
                return llite_paths[i];
        }

        ovis_log(mylog, OVIS_LWARNING, "no llite directories found\n");
        return NULL;
}
...
static int llites_refresh()
{
	...
	int err = 0;
        ...
        if (dir == NULL) {
		if (!dir_once_log) {
	                ovis_log(mylog, OVIS_LDEBUG, "unable to open llite dir %s\n",
	                       llite_path);
			dir_once_log = 1;
		}
                return 0;
                  ...
                  struct attr_value_list *kwl, struct attr_value_list *avl)
{
	...
	if (ival) {
		if (strlen(ival) < sizeof(producer_name)) {
			strncpy(producer_name, ival, sizeof(producer_name));
                        ...
                        ovis_log(mylog, OVIS_LERROR, SAMP": config: producer name too long.\n");
                        return EINVAL;
		}
	}
	(void)base_auth_parse(avl, &auth, mylog);
	...
	int cc = comp_id_helper_config(avl, &cid);
        if (cc) {
		ovis_log(mylog, OVIS_LERROR, SAMP": value of component_id="
			" is invalid.\n");
		return cc;
	}
        return 0;
...
static int sample(struct ldmsd_sampler *self)
{
        ...
        if (llite_general_schema_is_initialized() < 0) {
                if (llite_general_schema_init(&cid) < 0) {
                        ovis_log(mylog, OVIS_LERROR, "general schema create failed\n");
                        return ENOMEM;
                }
        }
