...
static struct mdc_data *mdc_create(const char *mdc_name, const char *basedir)
{
	...
	ovis_log(mylog, OVIS_LDEBUG, "mdc_create() %s from %s\n",
	       mdc_name, basedir);
...
{
	ovis_log(mylog, OVIS_LDEBUG, "mdc_destroy() %s\n", mdc->name);
...
static int mdcs_refresh()
{
	...
	if (dir == NULL) {
		if (!dir_once_log) {
			ovis_log(mylog, OVIS_LDEBUG, "unable to open mdc dir %s\n",
		  ...
		  struct attr_value_list *kwl, struct attr_value_list *avl)
{
	...
	if (je) {
		ovis_log(mylog, OVIS_LERROR, "job_set name too long\n");
		return ENAMETOOLONG;
	}
	...
	if (mdc_timing) {
		...
		if (!dchk) {
			...
			ovis_log(mylog, OVIS_LERROR, "use mdc_timing=0 or "
